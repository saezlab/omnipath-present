...
export async function getEntityFilterFacetDistributionPostgres(): Promise<FacetDistribution> {
  return loadFacetDistributionFromMaterializedView('entity_filter_counts');
}
...
export async function searchEntitiesPostgres(params: {
  ...
  try {
    ...
    const facetDistribution = offset === 0
      ? await getEntityFilterFacetDistributionPostgres()
      : undefined;
    return {
      hits,
