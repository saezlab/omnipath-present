...
export const entityRelation = pgTable("entity_relation", {
	...
	participantTypes: text("participant_types").array().default(["RAY"]).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const resources = pgTable("resources", {
	...
	categories: text().array().default(["RAY"]).notNull(),
	annotationOntologies: text("annotation_ontologies").array().default(["RAY"]).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
}).as(sql`SELECT entity_pk, count(DISTINCT relation_pk) AS relation_count FROM ( SELECT entity_relation.subject_entity_pk AS entity_pk, entity_relation.relation_pk FROM entity_relation UNION ALL SELECT entity_relation.object_entity_pk AS entity_pk, entity_relation.relation_pk FROM entity_relation) relation_endpoints GROUP BY entity_pk`);
...
export const ontologyTerms = pgMaterializedView("ontology_terms", {	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	termEntityPk: bigint("term_entity_pk", { mode: "number" }),
	termId: text("term_id"),
	...
	sources: text(),
}).as(sql`WITH term_entities AS ( SELECT e.entity_pk, e.canonical_identifier, e.sources, CASE WHEN jsonb_typeof(COALESCE(e.entity_attributes, '[]'::jsonb)) = 'array'::text THEN COALESCE(e.entity_attributes, '[]'::jsonb) ELSE '[]'::jsonb END AS attributes FROM entity e WHERE e.entity_type = 'OM:0012:Cv Term'::text AND e.canonical_identifier_type = 'OM:0204:Cv Term Accession'::text ), identifier_values AS ( SELECT te.entity_pk, (array_agg(ei.identifier ORDER BY ei.id) FILTER (WHERE ei.identifier_type = 'OM:0202:Name'::text AND COALESCE(ei.identifier, ''::text) <> ''::text))[1] AS name, array_agg(DISTINCT ei.identifier ORDER BY ei.identifier) FILTER (WHERE ei.identifier_type = 'OM:0203:Synonym'::text AND COALESCE(ei.identifier, ''::text) <> ''::text) AS synonyms FROM term_entities te LEFT JOIN entity_identifier ei ON ei.entity_pk = te.entity_pk GROUP BY te.entity_pk ), attribute_values AS ( SELECT te.entity_pk, (array_agg(attr.item ->> 'value'::text ORDER BY attr.ordinality) FILTER (WHERE (attr.item ->> 'term'::text) = 'OM:0202:Name'::text AND COALESCE(attr.item ->> 'value'::text, ''::text) <> ''::text))[1] AS name, (array_agg(attr.item ->> 'value'::text ORDER BY attr.ordinality) FILTER (WHERE (attr.item ->> 'term'::text) = 'OM:0801:Definition'::text AND COALESCE(attr.item ->> 'value'::text, ''::text) <> ''::text))[1] AS definition, (array_agg(attr.item ->> 'value'::text ORDER BY attr.ordinality) FILTER (WHERE ((attr.item ->> 'term'::text) = ANY (ARRAY['OM:0803'::text, 'OM:0803:Ontology Id'::text])) AND COALESCE(attr.item ->> 'value'::text, ''::text) <> ''::text))[1] AS ontology_id, array_agg(DISTINCT attr.item ->> 'value'::text ORDER BY (attr.item ->> 'value'::text)) FILTER (WHERE (attr.item ->> 'term'::text) = 'OM:0203:Synonym'::text AND COALESCE(attr.item ->> 'value'::text, ''::text) <> ''::text) AS synonyms FROM term_entities te LEFT JOIN LATERAL jsonb_array_elements(te.attributes) WITH ORDINALITY attr(item, ordinality) ON true GROUP BY te.entity_pk ) SELECT term_entity_pk, term_id, ontology_prefix, label, definition, ontology_id, synonyms, array_to_string(synonyms, ' '::text) AS synonyms_text, sources FROM ( SELECT te.entity_pk AS term_entity_pk, te.canonical_identifier AS term_id, CASE WHEN te.canonical_identifier ~* '^KW-[0-9]+$'::text THEN 'kw'::text ELSE lower(split_part(te.canonical_identifier, ':'::text, 1)) END AS ontology_prefix, COALESCE(iv.name, av.name, te.canonical_identifier) AS label, av.definition, av.ontology_id, COALESCE(ARRAY( SELECT DISTINCT synonym.value FROM unnest(COALESCE(iv.synonyms, '{}'::text[]) || COALESCE(av.synonyms, '{}'::text[])) synonym(value) WHERE COALESCE(synonym.value, ''::text) <> ''::text ORDER BY synonym.value), '{}'::text[]) AS synonyms, te.sources FROM term_entities te LEFT JOIN identifier_values iv ON iv.entity_pk = te.entity_pk LEFT JOIN attribute_values av ON av.entity_pk = te.entity_pk) terms`);